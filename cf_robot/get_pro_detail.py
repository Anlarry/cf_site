...
class Get_pro_detail:
    def __init__(self, url):
        ...
        self.time_lim = tag.contents[0].get_text() + ': ' + tag.contents[1]
        tag = bs.find('div', attrs={'class':'memory-limit'})
        self.mem_lim = tag.contents[0].get_text() + ': ' + tag.contents[1] 
        tag = bs.find('div', attrs={'class':'input-file'})
        self.in_file = tag.contents[0].get_text() + ': ' + tag.contents[1]
        tag = bs.find('div', attrs={"class":'output-file'})
        self.out_file = tag.contents[0].get_text() + ': ' + tag.contents[1] 
